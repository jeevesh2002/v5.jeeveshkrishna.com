...
export default function BlogContent({ posts }: { posts: PostMeta[] }) {
  ...
  const shown = searching
    ? posts.map((p) => ({ p, s: score(query, p) })).filter((x) => x.s > 0).sort((a, b) => b.s - a.s).map((x) => x.p)
    : posts;
      ...
      <div className="search-wrap" style={{ marginBottom: "2.5rem" }}>
        ...
        {searching && (
          <div style={{ position: "absolute", right: 0, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: "0.75rem" }}>
            <span style={{ fontSize: "0.75rem", color: "var(--text-4)" }}>{shown.length} of {posts.length}</span>
      ...
      {shown.length === 0 ? (
        ...
        <Link key={post.slug} href={`/blog/${post.slug}`} style={{
          ...
          <div style={{ display: "flex", gap: "0.625rem", marginBottom: "0.5rem", flexWrap: "wrap" }}>
            <span style={{ fontSize: "0.8125rem", color: "var(--text-4)" }}>{formatDate(post.date)}</span>
            <span style={{ color: "var(--border)" }}>·</span>
            <span style={{ fontSize: "0.8125rem", color: "var(--text-4)" }}>{post.readingTime} min read</span>
          </div>
          <h2 style={{ fontSize: "1rem", fontWeight: 600, color: "var(--text-1)", lineHeight: 1.35, marginBottom: post.excerpt ? "0.5rem" : 0 }}>
            {post.title}
          </h2>
          {post.excerpt && (
            <p style={{ fontSize: "0.875rem", color: "var(--text-3)", lineHeight: 1.65, marginBottom: post.tags.length ? "0.75rem" : 0 }}>
              {post.excerpt}
            </p>
          )}
          {post.tags.length > 0 && (
