...
function score(query: string, link: FlatLink): number {
  ...
  const t = link.title.toLowerCase(),
    d = link.description.toLowerCase(),
    s = link.source.toLowerCase(),
    c = link.category.toLowerCase();
  let sc = 0;
...
export default function ReadingContent({ sections }: { sections: InterestSection[] }) {
  ...
  const allLinks: FlatLink[] = sections.flatMap((s) =>
    s.links.map((l) => ({ ...l, category: s.title }))
  );
  const searching = query.trim().length > 0;

  const results: FlatLink[] = searching
    ? allLinks
        .map((l) => ({ l, s: score(query, l) }))
        .filter((x) => x.s > 0)
        .sort((a, b) => b.s - a.s)
        .map((x) => x.l)
    : [];
...
function LinkCard({
  ...
  link: { title: string; source: string; url: string; description: string };
}) {
  return (
    <a
      href={link.url}
    ...
    >
      ...
      <span style={{ fontSize: "0.75rem", color: "var(--text-4)", marginBottom: "0.5rem" }}>
        {link.source}
      </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "0.75rem", color: "var(--text-4)", flexShrink: 0 }}>
          {link.source}
        </span>
      </div>
